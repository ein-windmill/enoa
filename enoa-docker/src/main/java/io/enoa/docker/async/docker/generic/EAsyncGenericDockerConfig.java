...
public class EAsyncGenericDockerConfig {
  ...
  public <T> EnqueueAssetDocker<DRet<List<T>>> list(DIParser<List<T>> parser) {
    return EnqueueDocker.asset(this.docker._dockerconfig().executor(), () -> this.config.list(parser));
  }

  public <T> EnqueueAssetDocker<DRet<List<T>>> list(DIParser<List<T>> parser, DQPFilter dqp) {
    return EnqueueDocker.asset(this.docker._dockerconfig().executor(), () -> this.config.list(parser, dqp));
  }

  public <T> EnqueueAssetDocker<DRet<T>> create(DIParser<T> parser, String body) {
    return EnqueueDocker.asset(this.docker._dockerconfig().executor(), () -> this.config.create(parser, body));
  }

  public <T> EnqueueAssetDocker<DRet<T>> inspect(DIParser<T> parser, String id) {
    return EnqueueDocker.asset(this.docker._dockerconfig().executor(), () -> this.config.inspect(parser, id));
  }
