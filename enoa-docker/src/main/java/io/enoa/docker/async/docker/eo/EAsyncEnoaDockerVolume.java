...
public class EAsyncEnoaDockerVolume {
  ...
  public EnqueueAssetDocker<DRet<EVolumeLs>> list() {
    return EnqueueDocker.asset(this.docker._dockerconfig().executor(), () -> this.volume.list());
  }

  public EnqueueAssetDocker<DRet<EVolumeLs>> list(DQPVolumeList dqp) {
    return EnqueueDocker.asset(this.docker._dockerconfig().executor(), () -> this.volume.list(dqp));
  }

  public EnqueueAssetDocker<DRet<EVolume>> create(String body) {
    return EnqueueDocker.asset(this.docker._dockerconfig().executor(), () -> this.volume.create(body));
  }

  public EnqueueAssetDocker<DRet<EVolume>> create(DQPVolumeCreate dqp) {
    return EnqueueDocker.asset(this.docker._dockerconfig().executor(), () -> this.volume.create(dqp));
  }

  public EnqueueAssetDocker<DRet<EVolume>> inspect(String id) {
    return EnqueueDocker.asset(this.docker._dockerconfig().executor(), () -> this.volume.inspect(id));
  }
  ...
  public EnqueueAssetDocker<DRet<EVolumePrune>> prune() {
    return EnqueueDocker.asset(this.docker._dockerconfig().executor(), () -> this.volume.prune());
  }

  public EnqueueAssetDocker<DRet<EVolumePrune>> prune(DQPFilter dqp) {
    return EnqueueDocker.asset(this.docker._dockerconfig().executor(), () -> this.volume.prune(dqp));
  }
}
