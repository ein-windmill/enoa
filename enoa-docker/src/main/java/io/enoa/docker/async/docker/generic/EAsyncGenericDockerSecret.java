...
public class EAsyncGenericDockerSecret {


  private GenericDocker docker;
  private EGenericDockerSecret secret;

  EAsyncGenericDockerSecret(GenericDocker docker) {
    this.docker = docker;
    this.secret = docker.secret();
  }

  public <T> EnqueueAssetDocker<DRet<List<T>>> list(DIParser<List<T>> parser) {
    return EnqueueDocker.asset(this.docker._dockerconfig().executor(), () -> this.secret.list(parser));
  }

  public <T> EnqueueAssetDocker<DRet<List<T>>> list(DIParser<List<T>> parser, DQPFilter dqp) {
    return EnqueueDocker.asset(this.docker._dockerconfig().executor(), () -> this.secret.list(parser, dqp));
  }

  public <T> EnqueueAssetDocker<DRet<T>> create(DIParser<T> parser, String body) {
    return EnqueueDocker.asset(this.docker._dockerconfig().executor(), () -> this.secret.create(parser, body));
  }

  public <T> EnqueueAssetDocker<DRet<T>> inspect(DIParser<T> parser, String id) {
    return EnqueueDocker.asset(this.docker._dockerconfig().executor(), () -> this.secret.inspect(parser, id));
  }
