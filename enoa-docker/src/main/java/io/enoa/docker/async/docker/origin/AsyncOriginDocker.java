...
public class AsyncOriginDocker {

  private DockerConfig config;
  private OriginDocker docker;

  public AsyncOriginDocker(OriginDocker docker) {
    this.docker = docker;
    this.config = this.docker._dockerconfig();
  }

  public EnqueueDoneargDocker<DResp> info() {
    return EnqueueDocker.donearg(this.config.executor(), () -> this.docker.info());
  }
