 ...
 */
class LimitedServletInputStream extends EoxInputStream {
   ...
   */
  LimitedServletInputStream(EoxInputStream in, int totalExpected) {
    super(in);
    this.in = in;
...
//        result = ((BufferedServletInputStream) in).origin().readLine(b, off, Math.min(left, len));
//      } else {
