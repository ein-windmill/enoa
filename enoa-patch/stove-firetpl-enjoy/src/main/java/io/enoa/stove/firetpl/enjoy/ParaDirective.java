 ...
 *
 * 2： java 代码
 *   SectionPara sp = getSectionPara("find", Kv.by("nickName", "prettyGirl").set("age", 18));
 *   user.find(sp)
 *   或者：
 *   user.find(sp.getSection(), sp.getPara());
 *
 ...
 *
 * 2： java 代码
 *   SectionPara sp = getSectionPara("find", 10, 100);
 *   user.find(sp)
 *
...
public class ParaDirective extends Directive {
  ...
  public void exec(Env env, Scope scope, Writer writer) {
    SectionPara sectionPara = (SectionPara) scope.get(SectionKit.SECTION_PARA_KEY);
    if (sectionPara == null) {
      throw new TemplateException("#para directive invoked by getSectionPara(...) method only", location);
    }
    ...
    if (index == -1) {
      ...
      if (paras == null) {
        throw new TemplateException("The #para(" + index + ") directive must invoked by getSectionPara(String, Object...) method", location);
      }
