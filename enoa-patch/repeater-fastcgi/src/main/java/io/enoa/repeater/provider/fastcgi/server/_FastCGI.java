...
import io.enoa.repeater.provider.fastcgi.kernel.FastCGIServer;

import java.io.IOException;

class _FastCGI implements RepeaterServer {

  private EoxAccessor accessor;

  _FastCGI(EoxAccessor accessor) {
    this.accessor = accessor;
  }

  @Override
  public void listen(String hostname, int port, boolean ssl, EoxConfig config, EoxProviderFactory factory) {
    FastCGIServer server = new FastCGIServer(new _FastCGIApp(this.accessor, config, factory));
    try {
