...
class TomcatRequestWrapper implements Request {
  ...
  TomcatRequestWrapper(HttpServletRequest request, EoxConfig config, EoxNameRuleFactory rule) {
      ...
      try {
        ...
        for (Part part : this.request.getParts()) {
          ...
          if (config.holdFile()) {
            ...
            part.write(TextKit.union(this.config.tmp().toString(), File.separator, filename));
