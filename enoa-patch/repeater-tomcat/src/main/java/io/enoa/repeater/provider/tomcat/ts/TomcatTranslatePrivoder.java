...
public class TomcatTranslatePrivoder implements RepeaterTranslateFactory<HttpServletRequest, HttpServletResponse> {
  @Override
  public Request request(EoxConfig config, EoxNameRuleFactory rule, HttpServletRequest oreq) {

    try {
      String vendor = config.other().string("provider.tomcat.upload.vendor", "default");
      switch (vendor) {
        case "cos":
          return new TomcatCosRequestWrapper(oreq, config, rule);
        case "common-upload":
          throw new EoException("Can not support now.");
        default:
          return new TomcatRequestWrapper(oreq, config, rule);
      }
    } catch (Exception e) {
